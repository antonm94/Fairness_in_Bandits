...
def get_regret(n_average):
   r_theta = bandits.get_mean()
   distance_to_max = max(r_theta) - r_theta
   #print n_average[self.T-1]
   return np.sum(n_average * distance_to_max)
...
def simpleTest():
   ...
   bestAction = np.argmax(bandits.get_mean())
   #rewards = lambda choice, t: random.random() + biases[choice]
   rewards = lambda choice, t: bandits.pull(choice)
   cumulativeReward = 0
...
if __name__ == "__main__":
   ...
   numPlays = [1] * bandits.k
   print(simpleTest())
   print numPlays
   print get_regret(numPlays)
